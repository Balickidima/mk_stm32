...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
/* Константы для обнаружения двойного нажатия и дребезга */
#define ANTIBOUNCE_TIME_MS          20U    /* Время дребезга кнопки (мс) */
#define MAX_DOUBLE_CLICK_INTERVAL_MS 400U  /* Максимальный интервал между нажатиями для двойного нажатия (мс) */
#define MAX_CLICK_DURATION_MS      2000U   /* Максимальная длительность нажатия для учета как "клика" (не длинного нажатия) (мс) */
/* USER CODE END PD */
...
/* Новые переменные для спящего режима и обнаружения двойного нажатия */
volatile uint8_t v_rezhim_sna = 0;                              /* Флаг: 0 - бодрствуем, 1 - только что проснулись (игнорируем следующее нажатие) */
volatile uint32_t vremya_poslednego_nazhatiya_dlya_dvojnogo = 0; /* Время первого нажатия в текущей последовательности для обнаружения двойного нажатия */
volatile uint8_t schitatel_nazhatiy = 0;                       /* Счетчик нажатий в текущей последовательности (0,1,2...) */
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    {
      /* Если мы только что проснулись, игнорируем это нажатие и сбрасываем флаг */
      if (v_rezhim_sna == 1)
      {
        v_rezhim_sna = 0;
        /* Игнорируем это нажатие - не начинаем отсчет */
      }
      else
      {
    ...
    else if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_SET && vremya_nazhatiya_knopki != 0)
    {
      ...
      /* Обновление времени последнего нажатия для таймаута бездействия */
      vremya_poslednego_nazhatiya = HAL_GetTick();
      ...
      {
        NVIC_SystemReset(); /* Перезагрузка устройства при длинном нажатии более 5 секунд */
      }
      else
      {
        /* Обработка короткого нажатия для обнаружения одиночного и двойного клика */
        if (prodolzhitelnost_nazhatiya < MAX_CLICK_DURATION_MS)
        {
          /* Это валидное нажатие (не длинное) для учета в последовательности кликов */
          schitatel_nazhatiy++;
          if (schitatel_nazhatiy == 1)
          {
            /* Первое нажатие в последовательности - запоминаем время */
            vremya_poslednego_nazhatiya_dlya_dvojnogo = HAL_GetTick();
          }
          else if (schitatel_nazhatiy == 2)
          {
            /* Второе нажатие - проверяем интервал между первым и вторым */
            uint32_t interval = HAL_GetTick() - vremya_poslednego_nazhatiya_dlya_dvojnogo;
            if (interval < MAX_DOUBLE_CLICK_INTERVAL_MS)
            {
              /* Двойное нажатие detected - переходим в спящий режим */
              EnterSleepMode();
              /* После пробуждения сбрасываем счетчик и время первого нажатия */
              schitatel_nazhatiy = 0;
              vremya_poslednego_nazhatiya_dlya_dvojnogo = 0;
            }
            else
            {
              /* Второе нажатие слишком позднее - первое нажатие считается одиночным */
              ...
              /* Начинаем новую последовательность со второго нажатия как первого */
              schitatel_nazhatiy = 1;
              vremya_poslednego_nazhatiya_dlya_dvojnogo = HAL_GetTick();
            }
          }
          else /* schitatel_nazhatiy > 2 */
          {
            /* Более двух нажатий - начинаем новую последовательность с текущего нажатия как первого */
            schitatel_nazhatiy = 1;
            vremya_poslednego_nazhatiya_dlya_dvojnogo = HAL_GetTick();
          }
        }
        ...
        {
          /* Нажатие слишком длинное (но менее 5000 мс) - сбрасываем счетчик кликов */
          schitatel_nazhatiy = 0;
          vremya_poslednego_nazhatiya_dlya_dvojnogo = 0;
        }

        /* Проверка таймаута для первого нажатия (если мы ждем второго нажатия слишком долго) */
        if (schitatel_nazhatiy == 1 && (HAL_GetTick() - vremya_poslednego_nazhatiya_dlya_dvojnogo) > MAX_DOUBLE_CLICK_INTERVAL_MS)
        {
          /* Одиночное нажатие не было followed вторым в течение заданного интервала - переключаем PWM */
          modul_raboty_svetodioda = !modul_raboty_svetodioda;
          if (modul_raboty_svetodioda)
          {
            znachenie_duty_cikla = 0;
          ...
          /* Сбрасываем счетчик и время первого нажатия */
          schitatel_nazhatiy = 0;
          vremya_poslednego_nazhatiya_dlya_dvojnogo = 0;
        }
      }
...
static void MX_GPIO_Init(void)
{
  ...
  /* Настройка PA0 как wakeup pin для выхода из спящего режима (срабатывание по низкому уровню) */
...
/* USER CODE BEGIN 4 */
/**
 * @brief Функция подготовки и перехода в спящий режим (STOP) и восстановления после пробуждения
 * @note Выполняет подготовку к сну, вход в STOP режим через HAL_PWR_EnterSTOPMode,
 *       и после пробуждения восстанавливает системное тактовое генерирование и устанавливает флаг пробуждения
 * @retval None
 */
static void EnterSleepMode(void)
{
  /* Пользователь может добавить код подготовки к сну здесь (например, отключение ненужной периферии) */

  /* Вход в STOP режим с основным регулятором включенным для быстрого пробуждения */
  HAL_PWR_EnterSTOPMode(PWR_MAINREGULATOR_ON, PWR_STOPENTRY_WFI);

  /* После пробуждения восстанавливаем системное тактовое генерирование */
  SystemClock_Config();

  /* Указываем, что мы только что проснулись и должны игнорировать следующее событие нажатия кнопки */
  v_rezhim_sna = 1;
  /* Сбрасываем счетчик нажатий и время первого нажатия, чтобы не учесть пробуждающее нажатие как клик */
  schitatel_nazhatiy = 0;
  vremya_poslednego_nazhatiya_dlya_dvojnogo = 0;

  /* Дополнительное восстановление периферии при необходимости */
  /* Например, повторная инициализация GPIO если потребовалось */
}
/* USER CODE END 4 */

/**
 * @brief Функция восстановления после выхода из спящего режима
 * @note Восстанавливает системное тактовое генерирование и периферию после пробуждения
 * @retval None
 */
